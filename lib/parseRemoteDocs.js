...
const createNewPage = (html) => {
    ...
    let $mainBody = $subDoc('#main-content');
...
let processPages = (type, pages) => {
    ...
    return bPromise.map(pages, ({href, name}) => {
            ...
            return Promise.all([rp(`${awsDocumentationURL}/${href}.partial.html`), `${href}.html`, name]);
...
let getPageGroups = () => {
	return rp(tocFile)
		...
		.then(results => {
			...
			let resourceAndPropertyReferenceGroups = results[0]
